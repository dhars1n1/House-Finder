...
function initializeMap() {
    ...
    // Add markers for initial map_data if available
    if (typeof map_data !== 'undefined' && map_data.length > 0) {
        addMapMarkers(map_data);
        
        // If we have properties, zoom to fit them
        if (markers.length > 0) {
        ...
        if (listing.lat && listing.lng) {
            ...
            const popupContent = `
                <div class="map-popup">
                    ...
                    <a href="/listing/${listing.reference_code}" class="btn btn-sm btn-primary">View Details</a>
                </div>
            `;
